...
- **Search** (`bin/pycbc_inspiral`): strain conditioning (`pycbc/strain`) → template generation (`pycbc/waveform`, `pycbc/tmpltbank`) → matched filter (`pycbc/filter/matchedfilter.py`) → thresholding/clustering (`pycbc/events`) → chi-squared vetoes (`pycbc/vetoes`). **Status 2026-07: pycbc_inspiral runs end-to-end under `--processing-scheme cupy`** on the examples/search GW170814 data (`/work/yifanwang/gpu/search_gpu_test/run_inspiral.sh {cpu,cupy} out.hdf`): 99.8% of triggers match CPU 1:1, identical loudest event (GW170814, snr 9.4611), SNR rel diff ~3e-6. Bugs fixed to get there (all in `_cupy` prototype code, PR-worthy): undefined `inner_real`/`abs_arg_max_complex` in array_cupy; scalar reductions returning device arrays (now `.item()` host scalars, matching the CUDA backend's `.get()` convention); `backend='nvcc'` RawKernels (NVRTC works, no toolkit needed) and `#include <cstdint>` (NVRTC has no C++ stdlib); transposed grid/block launch config in `CUDAThresholdCluster` (shared-mem overflow → CUDA_ERROR_ILLEGAL_ADDRESS); `sinegauss.fd_sine_gaussian` copy=False on a host array. The chisq CPU-vs-GPU %-level trigger differences were investigated and are **benign**: identical inputs agree to ~1e-5 and the GPU kernel (exact integer-mod phase) is *closer* to fp64 truth than the CPU fp32 phasor recurrence; the trigger-level differences come entirely from fp32-rounding-level differences (~4e-6 median in-band) in the *data-estimated PSD*, amplified by the ill-conditioned equal-power bin-edge searchsorted near the template's high-frequency tail (edges shift by up to ~170 samples where power density → 0). Making bins bit-stable across backends would require computing the sigma_vec cumsum in float64 (cheap, optional upstream improvement). The never-exercised non-pow2 GPU shift_sum path had three latent bugs (extra `N` in signature, `*args` instead of `tuple(args)`, catastrophic fp32 `sincosf(phase*i)` at large i) — all fixed; both paths now agree with fp64 truth to ~3e-6. **ACCEPTANCE COMPLETE 2026-07-12: the full examples/search Pegasus workflow ran end-to-end with GPU inspiral jobs** (15/15 DAGs, 411 jobs, 0 failures; loudest coincident foreground = the H1L1V1 GW170814 triple at 1186741861.53, stat 34.8). Setup lives in `/work/yifanwang/gpu/search_workflow/`: `[inspiral] processing-scheme = cupy`, `[pegasus_profile-inspiral]` `condor|request_gpus = 1` + `condor|requirements = regexp("A100", CUDADeviceName)`, and `[executables] inspiral` pointing at a wrapper that MUST be named `pycbc_inspiral` (job-class dispatch is by basename) sourcing env-gpu. Traps hit (all infrastructure, not GPU math): statmap needs request_disk ≥ GBs; `[single_template]` in plotting.ini inherits the inspiral scheme via `${inspiral|...}` interpolation — followup jobs then demand cupy without GPUs (set it to cpu for GPU workflows); MKL absent in our venvs. Pegasus configs materialize in FOUR copies (source ini → generated minifollowup ini → output/daxes/ → local-site-scratch staged DAX, the one deferred replanning actually reads) plus planned .sh scripts — rescue-time fixes must patch .sh AND the scratch DAX.
  **examples/inference acceptance also complete** (2026-07-07): analytic-normal2d (bit-identical), single (distribution-consistent, ΔlogZ 0.017; GPU 11x slower — precompute+cheap-likelihood models have nothing to accelerate), relative (sample-identical). Pattern: `--processing-scheme cupy --nprocesses 1` (fork is CUDA-unsafe). env-gpu needs emcee==2.2.1 and dynesty==2.1.5 (pycbc wrappers reject emcee>=3 and dynesty 3 removed nestedsamplers).
